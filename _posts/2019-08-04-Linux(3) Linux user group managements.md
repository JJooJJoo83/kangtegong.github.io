...
기본적으로 root라는 이름을 가진 수퍼유저(Superuser) 가 있으며, 

root는 모든 작업을 할 수 있는 권한이 있다.

모든 사용자들은 하나 이상의 그룹에 소속되어 있는데, 
...
위와 같은 형식으로 사용자 계정이 정리되어 있음을 확인할 수 있다.
...
`vi /etc/group`에 정의되어 있다.   

	그룹이름 : 그룹 비밀번호 : 그룹 번호

위와 같은 형식으로 그룹이 정리 되어 있음을 확인할 수 있다.
...
그룹을 먼저 생성하지 않고 계정을 만들면 그 계정의 이름과 동일한 그룹을 자동으로 생성한 뒤, 이용자 계정을 그룹 안에 넣는 방식이기 때문에 디폴트로 이와 같이 지정된 것이다.
...
`-m N` 옵션 : 설정한 암호를 최소 N일은 써야 한다는 뜻
...
그룹의 암호를 설정하거나, 그룹의 관리를 수행하는 명령어이긴 한데,
그룹의 암호를 설정하는 경우가 많지는 않다.. 
...
비록 root일지라도 다른 사용자들의 비밀번호는 알 수 없다.

참고로 root가 리눅스시스템 전체에서 못하는 딱 하나가 이거다.
다른 사용자의 비밀번호를 아는 것.
...
만일 없다면 설치한 뒤 `system-config-user` 쳐서 들어가자
